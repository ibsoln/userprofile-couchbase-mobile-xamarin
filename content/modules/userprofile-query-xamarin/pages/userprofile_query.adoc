...
https://www.couchbase.com/products/n1ql[N1QL], Couchbase's declarative query language that extends
https://www.sqlite.org/index.html[SQL] for JSON.
...
https://en.wikipedia.org/wiki/Fluent_interface[Fluent API Design Pattern], and it uses method cascading to read to like a Domain Specific Language (DSL).
...
https://docs.couchbase.com/userprofile-couchbase-mobile/standalone/userprofile/xamarin/userprofile_basic.html[Fundamentals Tutorial] on using Couchbase Lite 2.0 as a standalone database
...
https://docs.couchbase.com/userprofile-couchbase-mobile/standalone/userprofile/xamarin/userprofile_basic.html[Fundamentals tutorial],
...
The
https://www.nuget.org/packages/Couchbase.Lite/[Couchbase.Lite Nuget package] is included as a reference within four projects of this solution:
...
* Open the
{githubsrc}/UserProfileDemo.Repositories/DatabaseManager.cs[DatabaseManager.cs] file and locate the `GetDatabaseAsync` method.
...
* In the
{githubsrc}/UserProfileDemo.Repositories/DatabaseManager.cs[*DatabaseManager.cs*] file,
locate the `CreateUniversitiesDatabaseIndexes` method.
...
The Query API in Couchbase Lite 2.0 is extensive.
...
* Open the
{githubsrc}/UserProfileDemo.Repositories/UniversityRepository.cs[*UniversityRepository.cs*] file and locate the `SearchByName` method.
+
[source,c#]
----
...
The <<University>> instance is populated with these property values.
