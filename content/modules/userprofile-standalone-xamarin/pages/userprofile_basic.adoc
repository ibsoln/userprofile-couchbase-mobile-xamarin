...
image::xamarin/user_profile.gif[{alt-text},200,400]
...
Assuming that you have link:https://git-scm.com/downloads[Git] installed you can do this using the following command:
+
[source,bash]
----
...
* Open the link:{githubsrc}/UserProfileDemo.Repositories/BaseRepository.cs[*BaseRepository.cs*] file and locate the `Database` property.
...
* We create an instance of the `DatabaseConfiguration` within link:{githubsrc}/UserProfileDemo.Repositories/UserProfileRepository.cs[*UserProfileRepository.cs*] via an `abstract` requirement from the parent class, `BaseRepository`.
...
* Open the link:{githubsrc}/UserProfileDemo.Repositories/BaseRepository.cs[*BaseRepository.cs*] file and locate the `Database.AddChangeListener` function usage within the constructor.
+
[source,c#]
----
...
Open the link:{githubsrc}/UserProfileDemo.Repositories/BaseRepository.cs[*BaseRepository.cs*] file and locate the `Dispose` method.
...
```
. Note the folder location of the database for the user, which is indicated in thee above log message
...
* Open the link:{githubsrc}/UserProfileDemo.Core/ViewModels/UserProfileViewModel.cs[*UserProfileViewModel.cs*] file and locate the `userProfileDocId` definition.
