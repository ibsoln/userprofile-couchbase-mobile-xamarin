...
:githubsrc: https://github.com/couchbaselabs/userprofile-couchbase-mobile-xamarin/tree/sync/content/modules/userprofile-sync-xamarin/examples/src
...
. Open the `UserProfileDemo.sln` file.
This is locate within `/path/to/UserProfileDemo/modules/userprofile/examples/src`.
+
[source,bash]
----
...
While it is neither required nor enforced, it is recommended practice to include a _"type"_ property that can serve as a namespace for related documents.
...
* To see the code behind this, open the
{githubsrc}/UserProfileDemo.Repositories/DatabaseManager.cs[*DatabaseManager.cs*] file and locate the `Start` method.
+
[source,c#]
----
include::example$/src/UserProfileDemo.Repositories/DatabaseManager.cs[tags=startSync,indent=0]
----
...
[source,c#]
----
include::example$/src/UserProfileDemo.Repositories/DatabaseManager.cs[tags=replicationconfig,indent=0]
----
+
<1> Initialize with `Source` as the local Couchbase Lite database and the `remote` target as the Sync Gateway
<2> Replication `type` of `PushAndPull` indicates that we require two-way sync.
...
[source,c#]
----
include::example$/src/UserProfileDemo.Repositories/DatabaseManager.cs[tags=replicationinit,indent=0]
----
...
[source,c#]
----
include::example$/src/UserProfileDemo.Repositories/DatabaseManager.cs[tags=replicationlistener,indent=0]
----
...
[source,c#]
----
include::example$/src/UserProfileDemo.Repositories/DatabaseManager.cs[tags=replicatorupdate,indent=0]
----
...
[source,c#]
----
include::example$/src/UserProfileDemo.Repositories/DatabaseManager.cs[tags=replicationstart,indent=0]
----
...
* Open the
{githubsrc}/UserProfileDemo.Repositories/DatabaseManager.cs[*DatabaseManager.cs*] file and locate the `Stop` function.
+
[source,c#]
----
include::example$/src/UserProfileDemo.Repositories/DatabaseManager.cs[tags=replicationstop,indent=0]
----
+
...
[source,c#]
----
include::example$/src/UserProfileDemo.Repositories/DatabaseManager.cs[tags=replicationcleanup,indent=0]
----
...
. Open the
{githubsrc}/UserProfileDemo.Repositories/UserProfileRepository.cs[*UserProfileRepository.cs*] file
...
[source,c#]
----
include::example$/src/UserProfileDemo.Repositories/UserProfileRepository.cs[tags=getUserLiveQuery,indent=0]
----

. Build the Query using `QueryBuilder` API.
+
If you are unfamiliar with this API, please check out this
https://developer.couchbase.com/documentation/mobile/2.0/userprofile_query.html[tutorial].
+
[source,c#]
----
include::example$/src/UserProfileDemo.Repositories/UserProfileRepository.cs[tags=livequerybuilder,indent=0]
----
...
[source,c#]
----
include::example$/src/UserProfileDemo.Repositories/UserProfileRepository.cs[tags=livequery, indent=0]
----
+
<1> Attach a listener callback to the query.
Attaching a listener automatically makes it _live_.
So any time there is a change in the user profile data in the underlying database, the callback will be invoked.
<2> The `SelectResult.all()` method is used to query all the properties of a document.
In this case, the document in the result is embedded in a dictionary where the key is the database name, which is _"userprofiles"_.
So, we retrieve the
http://docs.couchbase.com/mobile/2.0/couchbase-lite-swift/Classes/DictionaryObject.html[`DictionaryObject`] at key _"userprofiles"_.
<3> Create an instance of <<UserProfile>>.
...
. Open the command terminal and issue the following command to get the user profile document via
...
----
<.> If you updated an image via the mobile app, you should see an *"_attachments"* property.
...
. In the command terminal, issue the following command to update the user profile document via
[PUT Document REST API]
+
[source,bash]
----
curl -X PUT \
